    ...
    <form autocomplete="off" class="flex flex-col w-full absolute top-2/3 left-1/2 -translate-x-1/2 -translate-y-1/2 p-8 sm:max-w-[600px] gap-6 border rounded" action="{{route("book.store")}}" method="POST">
            ...
            <div class="flex flex-col sm:flex-row w-full gap-4">
                <div class="flex flex-col w-full">
                    ...
                    <input placeholder="ex: Harry Potter" class="w-full rounded border border-gray-300 py-2 px-4" type="text" name="title" id="title" required />
                </div>
                <div class="flex flex-col w-full gap">
                    <label for="address" class="mb-2">Adresse</label>
                    <input class="w-full rounded border border-gray-300 py-2 px-4" type="text" name="address" id="address" placeholder="ex: D1" required /></div>
            </div>
            <div class="w-full flex flex-col">
                <label for="category" class="mb-2">Categorie</label>
                <input class="w-full rounded border border-gray-300" type="text" name="category" id="category" placeholder="ex: Roman" required />
            </div>
                
            <div class="w-full flex flex-col">
                <label for="summary" class="mb-2">Resumé</label>
                <textarea placeholder="ex: inspiré de l'histoire de ..." class="w-full h-52 rounded border border-gray-300 p-4" type="text" name="summary" id="summary" required></textarea>
            </div>

            <div class="flex items-center">
                <label class="w-max" for="special">Livre special</label>
                <input type="checkbox" name="special" id="special" class="rounded ml-2">
            </div>
            ...
            <div class="w-full flex flex-col">
                ...
                <div id="autocomplete" class="w-full relative inline-block" data-authors="{{json_encode($authors)}}">
                    <input placeholder="ex: Napoleon Hill" id="author" name="author" class="w-full rounded border border-gray-300 py-2 px-4" required>
                </div>
            </div>
